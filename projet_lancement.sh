...
launch_gnbs() {
    ...
    for i in $(seq 1 $NUM_GNBS); do
        gnome-terminal -- bash -c "cd ./srsRAN_Project/build/apps/gnb/; echo $PASSWORD | sudo -S ./gnb -c gnb${i}0.yaml e2; exec bash"
...
send_to_influxdb() {
    ...
    gnome-terminal -- bash -c "python3.11 send_influxdb.py; exec bash"
}
...
launch_gnuradio() {
    ...
    gnome-terminal -- bash -c "cd ./gnuradio; gnuradio-companion ./${NUM_GNBS}gnb_scenario${NUM_UES}.grc; exec bash"
}

launch_metrics_xapp(){	
    echo "Lancement de l'xapp...."
    gnome-terminal -- bash -c "cd ./oran-sc-ric/xApps/python/; echo $PASSWORD |sudo -S docker compose exec python_xapp_runner2 ./kpm_mon_xapp.py; exec bash"
}
